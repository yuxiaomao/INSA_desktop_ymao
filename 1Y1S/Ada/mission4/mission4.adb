...
with Assert , Avion_Sol , INSA_Air ,Train , Pilote_Automatique,Tour,Carburant,Cartographie  ;
...
procedure Mission4 is
   ...
   function Naviguer_Vers(Code:String) return Float is
      Distance_Moyenne_Vers_Aeroport_Plus_proche:Float:=0.0;
      N:Float:=0.0;
      Distance_Avion_Aero_Plus_Proche:Float;
      Distance_Max_Avion_Aero_Plus_Proche:Float:=0.0;
   begin
      
      while Distance(Point1=>Cartographie.Coords_Avion , 
		     Point2 => Cartographie.Coords_Aeroport(Code=>Code)) > 100000.0 loop
	 
         Distance_Avion_Aero_Plus_Proche:=Distance(Point1=>Cartographie.Coords_Avion ,
			    Point2=>Cartographie.Coords_Aeroport(Code=>Code_Aeroport_Plus_Proche));
	 
         Orienter_En_Vol(Cap=>Cap_Orienter (Code=>Code));
     
	 Cartographie.Placer_Marque(Point=>Cartographie.Coords_Aeroport(Code=>Code_Aeroport_Plus_Proche ));
	 Distance_Moyenne_Vers_Aeroport_Plus_proche:= Distance_Moyenne_Vers_Aeroport_Plus_proche + Distance_Avion_Aero_Plus_Proche;
	 N:=N + 1.0;
	 
	 if Distance_Max_Avion_Aero_Plus_Proche < Distance_Avion_Aero_Plus_Proche then
	    Distance_Max_Avion_Aero_Plus_Proche:=Distance_Avion_Aero_Plus_Proche;
	 end if;
      ...
      GAda.Text_IO.Put_Line(Aff=>"Durant ce voyage,la distance moyenne entre avion et Aeroport plus proche est "
			      & Float'Image(Distance_Moyenne_Vers_Aeroport_Plus_proche / N)
			      & " M.");
      
      return Distance_Max_Avion_Aero_Plus_Proche;
      
   end Naviguer_Vers;
   
   
   --
   --mission4,afficher tous les aeroports connus avec leur numero
   --
   procedure Afficher_La_Liste_Des_Aeroports_Connus is
   begin
      Gada.Text_Io.Put_Line(Aff=> " Liste des Aero connu ");
      for N in 1 .. Cartographie.Nb_Aeroports loop
	 Gada.Text_Io.Put_Line(Aff=> Integer'Image(N) & " "
			       & Cartographie.Code_Aeroport ( Numero=> N ) &" "
				 & Cartographie.Nom_Aeroport(Code=>Cartographie.Code_Aeroport(Numero => N ))
			      & Cartographie.Pays_Aeroport(Code=>Cartographie.Code_Aeroport(Numero => N )));
      end loop;

   end Afficher_La_Liste_Des_Aeroports_Connus;
   ...
   --mission4
   --
   procedure Naviguer_Vers_Aeroport_Indique is
      N:Integer:=0;
      Dis_Max:Float:=0.0;
      Dis_Variable:Float:=0.0;
   begin
      GAda.Text_IO.Put_Line(Aff=>" Vous voulez vers ou? (tapez numero de l'aeroport , tapez 0 pour rentrer.) ");
      GAda.Integer_Text_IO.Get(Item => N);
      while N /= 0 loop
 
	 Dis_Variable:=Naviguer_Vers(Code=>Cartographie.Code_Aeroport(Numero=> N ));
	 if Dis_Max < Dis_Variable then
	    Dis_Max := Dis_Variable;
	 end if;
	 GAda.Text_IO.Put_Line(Aff=> "Nous sommes en train de aller vers : " );
         Gada.Text_IO.Put_Line(Aff=> Integer'Image(N) & " "
				 & Cartographie.Code_Aeroport( Numero=> N ) &" "
				 & Cartographie.Nom_Aeroport(Code=>Cartographie.Code_Aeroport(Numero => N )));
	 ...
	 GAda.Text_IO.Put_Line(Aff=>" Vous voulez vers ou? (tapez numero de l'aeroport , tapez 0 pour rentrer.) ");
       ...
       Dis_Variable:=Naviguer_Vers(Code=>"LFBO");
       if Dis_Max < Dis_Variable then
	  Dis_Max := Dis_Variable;
       end if;
       GAda.Text_IO.Put_Line(Aff=>"Durant tout voyage,l'avion a toujours ete a moins de "
			       & Float'Image(Dis_Max)
			       & " KM d'un aeroport");
       
   end Naviguer_Vers_Aeroport_Indique;
   ...
   begin
      ...
      Afficher_La_Liste_Des_Aeroports_Connus;
      Naviguer_Vers_Aeroport_Indique;
