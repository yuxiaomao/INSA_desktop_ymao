...
public class DessinVisible extends Dessin {
	...
	public DessinVisible(int largeur, int hauteur) {
		...
		BufferedImage img = new BufferedImage(largeur, hauteur,
	...
	public void setBB(double long1, double long2, double lat1, double lat2) {
		...
		/*
		 * Adapte la BB en fonction de la taille du dessin, pour préserver le
		...
		/*
		 * On ne peut qu'agrandir la BB, pour ne rien perdre. Si le ratiobb est
		 * trop petit, il faut agrandir deltalong s'il est trop grand, il faut
		 * agrandir deltalat.
		 */
		if (ratiobb < ratiogr) {
	...
	private void checkBB() {
		if (!this.bb_is_set) {
			throw new Error(
					"Classe DessinVisible : vous devez invoquer la methode setBB avant de dessiner.");
		}
	}
