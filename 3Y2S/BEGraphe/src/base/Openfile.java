...
public class Openfile {
	...
	// car il permet de tester en etant a l'INSA.
			...
			// ici.
			"D:/COURS/COURS 3MIC/S2/BE Graphes/PGraphe/Maps", };

	// Extension testees. Garder l'extension vide dans la liste.
	private static final String[] extensions = { ".map", ".gz", ".map.gz",
			".path", ".path.gz", "" };
	...
	public static DataInputStream open(String filename) {
		...
		if (!filename.equals(new File(filename).getName())) {
			System.out
					.println("Le nom du fichier ne doit pas contenir un chemin (ni absolu, ni relatif).");
			System.out
					.println("Il doit juste contenir le nom du fichier contenant la carte.");
			System.out
					.println("Si vous voulez utiliser un dossier specifique, configurez base/Openfile.java");
			System.exit(1);
		}
		...
		if (!trouve) {
			...
			for (int i = 0; i < datadirs.length; i++) {
				...
				if (!new File(datadirs[i]).isDirectory()) {
					...
					case 0:
						System.out
								.println("       (Ce dossier n'existe pas d'ailleurs)");
						break;
					case 1:
						System.out
								.println("       (Ce dossier n'existe pas non plus)");
						break;
					default:
