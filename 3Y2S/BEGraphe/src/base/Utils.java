...
public class Utils {
	...
	public static void checkByte(int val, DataInputStream dis)
			throws IOException {
		int octet = dis.readUnsignedByte();
		if (octet != val) {
			System.out.println("checkByte error : octet attendu = " + val
					+ "   octet lu = " + octet);
			System.exit(1);
		}
	}
	...
	public static void checkVersion(int magic, int magic_attendu, int version,
			int version_attendue, String nomFichier, String nom_format) {
		if (magic != magic_attendu) {
			System.out.println("Le fichier " + nomFichier
					+ " n'est pas au format " + nom_format);
			System.exit(1);
		}
		...
		if (version != version_attendue) {
			if (version > version_attendue) {
				System.out
						.println("La classe Graphe doit etre mise a jour pour lire le fichier "
								+ nomFichier);
			} else {
				System.out.println("Le fichier " + nomFichier
						+ " est dans un format trop ancien");
			}
			System.out.println(" (version Graphe.java = " + version_attendue
					+ " (pour la lecture des " + nom_format + "), version de "
					+ nomFichier + " = " + version);
			System.exit(1);
		}
	}
